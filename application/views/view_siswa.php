...
<div class="container-fluid">
  ...
  <tbody>
    ...
    <tr>
      ...
      <a href="<?php echo base_url(); ?>siswa/hapus/<?php echo $row['nis']; ?>" class="btn btn-danger" onclick="return confirm('Yakin Mau di Hapus?');" >Delete</a></td>
    <?php } ?>
    </tr>
  </tbody>
</table>
  ...
  <div class="mb-3">
    ...
    <select name='kelas' class='form-control'custome="custom-select">
      <?php foreach ($kelas->result_array() as $row){
        ?>
        <option value="<?php echo $row['kode_kelas']?>"><?php echo $row['nama_kelas']?></option><?php }?>
      }
    </select>
  </div>
  ...
  <div class="modal fade" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
     <div class="modal-dialog modal-xl">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="exampleModalLabel">Edit Siwa</h5>
                 <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                 </button>
             </div>
